...
public sealed class CotizacionesService(MarketDatabase database)
{
    private static readonly JsonSerializerOptions JsonOptions = new(JsonSerializerDefaults.Web);

    private List<(DateTime Fecha, List<T> Items)> Read<T>(string table, int count)
    {
        if (!MarketDatabase.SnapshotTables.Contains(table)) throw new ArgumentException("Unknown snapshot type", nameof(table));
